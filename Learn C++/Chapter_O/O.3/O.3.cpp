...
int main()
{
    ...
    flags |= (mask1 | mask2); // turn on bit 1 & bit 2 at the same time
    
    std::cout << "bit 1 is " << (static_cast<bool>(flags & mask1) ? "on\n" : "off\n");

    flags &= ~(mask1 | mask2); // turn off bit 1 & bit2 at the same time
    ...
    std::cout << "bit 2 is " << (static_cast<bool>(flags & mask2) ? "on\n" : "off\n");
    flags ^= mask2; // flip bit2
    std::cout << "bit 2 is " << (static_cast<bool>(flags & mask2) ? "on\n" : "off\n");

    return 0;
}

/* "&" is used to see if a bit is on or off and use the "flags" bitmask to compare against
*/